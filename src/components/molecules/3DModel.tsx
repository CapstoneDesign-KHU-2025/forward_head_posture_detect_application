...
export default function ThreeDModel() {
  ...
  useEffect(() => {
    ...
    function initSceneAndCamera() {
      ...
      camera.position.set(-1.8, 1.9, 0.25);
    }
    ...
    function initAngleGuides() {
      ...
      guideLine.visible = false; // 이전 가이드는 숨김
      scene.add(guideLine);
    ...
    function initUI() {
      ...
      // world/landmarks 토글(좌상단 그대로 두되, 안 써도 됨)
    ...
    function updateFrame(t: number, dt: number): void {
      ...
      // 4) 연결선(원래 포즈 라인) 갱신 (지금은 안 보이지만 로직은 유지)
      rebuildLinesNow();
    }
