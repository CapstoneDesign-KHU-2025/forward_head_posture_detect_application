...
export default function ThreeDemModel() {
  ...
  useEffect(() => {
    ...
    (controls as any).zoomToCursor = true; // d.ts에 없을 수 있어 캐스트
    ...
    /* 숨길 포인트(손가락) */
    const HIDE_INDICES = new Set<number>([17,18,19,20,21,22]);
    ...
    function applyVisibilityForMode(mode: PoseMode): void {
      ...
      for (let i=13;i<=32;i++){
        jointSpheres[i].visible = !hideLower && !HIDE_INDICES.has(i);
      }
    }
    ...
    function applyBasePose(mode: PoseMode = 'stand'): void {
      ...
      if(mode==='stand'){
        ...
        const shoulderY = 1.45, shoulderZ = 0.0;
        ...
        const hipY = 0.9, hipZ = 0.0, hipHalf2 = 0.14;
    ...
    function updateFrame(t: number, dt: number): void {
      ...
      const absYellowWorldDeg = Math.max(0, THREE.MathUtils.radToDeg(
        Math.atan2(yellowWorld.dot(worldUp), yellowWorld.dot(worldFwd))
      ));
    ...
    window.setHeadFromMediapipe = (landmarksOrResult: PoseResult | Landmark[], opts: { useWorld?: boolean; zFlip?: boolean; scale?: number } = {}) => {
      ...
      const pts: THREE.Vector3[] = [];
      for (let i=0;i<=10;i++){
        const p = lm[i]; if (!p) { pts.push(new THREE.Vector3(0,0,0)); continue; }
    ...
    (function makeLMToggleUI(){
      ...
      window.__anglePanel = (() => {
        ...
        const title = document.createElement('div'); title.textContent = 'Neck Angle';
        ...
        return {
          set(absDeg: number, deltaDeg: number, idealDeg: number){
            ...
            const warn = Math.abs(deltaDeg) >= 15 ? 'rgba(255,120,120,0.9)'
      ...
      const mkBtn = (t: string)=>{ const b=document.createElement('button'); b.textContent=t;
        Object.assign(b.style,{marginRight:'6px',padding:'6px 10px',borderRadius:'8px',
      ...
      function apply(useWorld: boolean){
        ...
        window.setBodyFromMediapipe?.({ landmarks: src.landmarks, worldLandmarks: src.worldLandmarks }, {
          useWorld: !!useWorld,
          yFlip: true,
          zFlip: true,
          scale: 1.0,
    ...
    (function makePoseToggleUI(){
      ...
      Object.assign(btn.style,{padding:'6px 12px',cursor:'pointer',
    ...
    return () => {
      ...
      Array.from(containerRef.current?.children || []).forEach((el) => { if (el !== renderer.domElement) el.remove(); });
      renderer.dispose();
      sphereGeom.dispose();
      lineGeom.dispose();
  ...
  return <div ref={containerRef} style={{ position:'relative', width:'100%', height:'100vh', overflow:'hidden' }} />;
}
